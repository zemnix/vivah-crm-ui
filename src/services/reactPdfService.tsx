...
import type { ServiceRequest } from '@/api/serviceRequestApi';
...
const stylesICSR = StyleSheet.create({
  fieldName: {
    width: "50%",
    fontSize: 10,
    fontFamily: 'Helvetica-Bold',
...
// ICSR Document Component
interface ICSRDocumentProps {
  serviceRequest: ServiceRequest;
  business?: Business;
}

const ICSRDocument: React.FC<ICSRDocumentProps> = ({ serviceRequest }) => {
  const formatDate = (dateString: string) => {
  ...
  const formatDateTime = (dateString: string) => {
    // format date time to dd/mm/yyyy hh:mm AM/PM i.e. 09/10/2025 09:00 AM
    if (!dateString) return new Date().toLocaleString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true });
    try {
      return new Date(dateString).toLocaleString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true });
    } catch {
      return new Date().toLocaleString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true });
    }
  };
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page} wrap>
        {/* FIXED HEADER - Same as quotation */}
        <View style={styles.headerFixed} fixed>
          <View style={styles.companyLogoGSTINSection}>
            <View>
              <Image src='/vinayak_industries_logo.jpeg' style={styles.companyLogo} />
            </View>
            <View>
              <Text>GSTIN NO: 21HCGPS1097Q2ZV</Text>
            </View>
        ...
        {/* FIXED FOOTER - Same as quotation */}
        <View style={styles.footerFixed} fixed>
          <Text style={styles.footerAddress}>
            Plot No: 546/1960, Near Mo bus depot, Patia Station Rd, Bhubaneswar- 751024, Khurdha, Odisha
          </Text>
        ...
        {/* WATERMARK */}
        <Image src='/vinayak_industries_logo.jpeg' style={styles.watermark} fixed />

        {/* MAIN CONTENT */}
        <View style={styles.mainContent}>
          {/* ICSR Title */}
          <Text style={{ ...styles.quotationTitle, marginBottom: 25 }}>
            INITIAL CUSTOMER SERVICE REPORT
          </Text>
          ...
          <View style={styles.table}>
            {/* Table Header */}
            <View style={styles.tableHeader}>
              <Text style={{ width: "100%", borderRight: '1px solid black', textAlign: 'center', fontFamily: 'Helvetica-Bold', fontSize: 12, padding: 4 }}>CUSTOMER DETAILS</Text>
            </View>

            {/* Table Rows */}

            <View style={styles.tableRow}>
              <Text style={stylesICSR.fieldName}>Customer Name</Text>
              <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).customerDetails?.name || 'N/A'}</Text>
            </View>

            <View style={styles.tableRow}>
              <Text style={stylesICSR.fieldName}>Address</Text>
              <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).customerDetails?.address || 'N/A'}</Text>
            </View>

            <View style={styles.tableRow}>
              <Text style={stylesICSR.fieldName}>Mobile</Text>
              <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).customerDetails?.mobile || 'N/A'}</Text>
            </View>
            ...
            <View style={{ marginTop: 20 }}>
              ...
              <View style={styles.table}>
                {/* Table Header */}
                <View style={styles.tableHeader}>
                  <Text style={{ width: "100%", borderRight: '1px solid black', textAlign: 'center', fontFamily: 'Helvetica-Bold', fontSize: 12, padding: 4 }}>PROBLEM DETAILS</Text>
                </View>
                {/* Table Rows */}
                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Problem Reported</Text>
                  <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).problemDetails?.problemReported || 'N/A'}</Text>
                </View>

                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Machine Name</Text>
                  <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).problemDetails?.machineName || 'N/A'}</Text>
                </View>

                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Location of Installation</Text>
                  <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).problemDetails?.locationOfInstallation || 'N/A'}</Text>
                </View>

                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Report Date & Time</Text>
                  <Text style={stylesICSR.fieldValue}>{formatDateTime((serviceRequest as any).problemDetails?.reportDateTime || (serviceRequest as any).createdAt)}</Text>
                </View>

                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Reported By</Text>
                  <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).createdBy?.name || 'N/A'}</Text>
                </View>
              </View>
            </View>
          </View>
...
// FCSR Document Component
interface FCSRDocumentProps {
  serviceRequest: ServiceRequest;
  business?: Business;
}

const FCSRDocument: React.FC<FCSRDocumentProps> = ({ serviceRequest }) => {
  const formatDate = (dateString: string) => {
    if (!dateString) return new Date().toLocaleDateString('en-IN');
    try {
      return new Date(dateString).toLocaleDateString('en-IN');
        ...
        <View style={styles.mainContent}>
          ...
          <View style={{ marginTop: 20 }}>
            <View style={styles.table}>
              {/* Table Header */}
              <View style={styles.tableHeader}>
                <Text style={{ width: "100%", borderRight: '1px solid black', textAlign: 'center', fontFamily: 'Helvetica-Bold', fontSize: 12, padding: 4 }}>SERVICE DETAILS</Text>
              </View>
              {/* Table Rows */}
              {(serviceRequest as any).problemDetails?.resolvedBy && (
              <View style={styles.tableRow}>
                <Text style={stylesICSR.fieldName}>Resolved By</Text>
                <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).problemDetails?.resolvedBy?.name || 'N/A'}</Text>
              </View>
              )}

              {(serviceRequest as any).problemDetails?.resolvedBy && (
              <View style={styles.tableRow}>
                <Text style={stylesICSR.fieldName}>Resolved By Mobile</Text>
                <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).problemDetails?.resolvedBy?.mobile || 'N/A'}</Text>
              </View>
              )}

              <View style={styles.tableRow}>
                <Text style={stylesICSR.fieldName}>Service Type</Text>
                <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).serviceCharges?.serviceType?.toUpperCase() || 'N/A'}</Text>
              </View>

              {(serviceRequest as any).serviceCharges?.serviceType === 'paid' && (serviceRequest as any).serviceCharges?.serviceCharge && (
                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Service Charge</Text>
                  <Text style={stylesICSR.fieldValue}>₹{(serviceRequest as any).serviceCharges.serviceCharge}</Text>
                </View>
              )}

              <View style={styles.tableRow}>
                <Text style={stylesICSR.fieldName}>Resolution Date & Time</Text>
                <Text style={stylesICSR.fieldValue}>{formatDateTime((serviceRequest as any).problemDetails?.resolvedDateTime || (serviceRequest as any).updatedAt)}</Text>
              </View>

              {(serviceRequest as any).problemDetails?.resolutionNotes && (
                <View style={styles.tableRow}>
                  <Text style={stylesICSR.fieldName}>Resolution Notes</Text>
                  <Text style={stylesICSR.fieldValue}>{(serviceRequest as any).problemDetails.resolutionNotes}</Text>
                </View>
              )}
            </View>
            ...
            <View  style={{ marginTop: 30 }}>
              <Image src='/vi_qr.png' style={{ width: 150, height: 150, marginBottom: 16, alignSelf: 'center' }} />
              <View>
                <Text style={{ fontSize: 12, fontFamily: 'Helvetica-Bold', textAlign: 'center' }}>PLEASE REVIEW US 5 STARS</Text>
              </View>
            </View>
          </View>
        </View>
...
export class ReactPDFService {
  ...
  // ICSR PDF Methods
  static async generateICSRPDF(serviceRequest: ServiceRequest, business?: Business) {
    try {
      console.log('ReactPDFService: Generating ICSR PDF...');
      const doc = <ICSRDocument serviceRequest={serviceRequest} business={business} />;
      const blob = await pdf(doc).toBlob();
      console.log('ReactPDFService: ICSR PDF generated successfully');
      return blob;
    } catch (error) {
      console.error('ReactPDFService: Error generating ICSR PDF:', error);
      throw new Error(`ICSR PDF generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  static async downloadICSRPDF(serviceRequest: ServiceRequest, business?: Business, options: { filename?: string } = {}) {
    try {
      console.log('ReactPDFService: Starting ICSR PDF download...');
      const filename = options.filename || `icsr-${serviceRequest._id.slice(-6)}-${Date.now()}.pdf`;
      const pdfBlob = await this.generateICSRPDF(serviceRequest, business);
      ...
      URL.revokeObjectURL(url);
      console.log('ReactPDFService: ICSR PDF download completed successfully');
    } catch (error) {
      console.error('ReactPDFService: Error downloading ICSR PDF:', error);
      throw error;
    }
  }

  static async previewICSRPDF(serviceRequest: ServiceRequest, business?: Business) {
    try {
      const pdfBlob = await this.generateICSRPDF(serviceRequest, business);
      return URL.createObjectURL(pdfBlob);
    } catch (error) {
      console.error('ReactPDFService: Error generating ICSR preview:', error);
      throw error;
    }
  }

  // FCSR PDF Methods
  static async generateFCSRPDF(serviceRequest: ServiceRequest, business?: Business) {
    try {
      console.log('ReactPDFService: Generating FCSR PDF...');
      const doc = <FCSRDocument serviceRequest={serviceRequest} business={business} />;
      const blob = await pdf(doc).toBlob();
      console.log('ReactPDFService: FCSR PDF generated successfully');
      return blob;
    } catch (error) {
      console.error('ReactPDFService: Error generating FCSR PDF:', error);
      throw new Error(`FCSR PDF generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  static async downloadFCSRPDF(serviceRequest: ServiceRequest, business?: Business, options: { filename?: string } = {}) {
    try {
      console.log('ReactPDFService: Starting FCSR PDF download...');
      const filename = options.filename || `fcsr-${serviceRequest._id.slice(-6)}-${Date.now()}.pdf`;
      const pdfBlob = await this.generateFCSRPDF(serviceRequest, business);
      ...
      URL.revokeObjectURL(url);
      console.log('ReactPDFService: FCSR PDF download completed successfully');
    } catch (error) {
      console.error('ReactPDFService: Error downloading FCSR PDF:', error);
      throw error;
    }
  }

  static async previewFCSRPDF(serviceRequest: ServiceRequest, business?: Business) {
    try {
      const pdfBlob = await this.generateFCSRPDF(serviceRequest, business);
      return URL.createObjectURL(pdfBlob);
    } catch (error) {
      console.error('ReactPDFService: Error generating FCSR preview:', error);
      throw error;
    }
  }
}
